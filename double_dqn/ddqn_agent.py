...
import random
import gym_ple
# This is the implementation of ddqn
# 2017-01-19
# author: Tianhong Dai
...
class ddqn_brain:
	...
	def train_network(self):
		...
		while True:
			...
			if num_of_episode % 100 == 0:
				save_path = self.save_dir + 'policy_model_' + str(num_of_episode) + '.pt'
	...
	def update_network(self, mini_batch):
		...
		_, _, aciton_max = self.deep_q_network(state_next_batch_tensor)
		aciton_max = aciton_max.unsqueeze(1)
		aciton_max = aciton_max.detach()

		# expected Q value from the target network...
		Q_value_expected, _, _ = self.target_network(state_next_batch_tensor)
		Q_value_expected = Q_value_expected.gather(1, aciton_max)
		Q_value_expected = Q_value_expected.detach()

		target = reward_batch_tensor + self.gamma * Q_value_expected * done_batch_tensor
